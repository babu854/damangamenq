...
function WinOrLossPopup({ gameName, totalBet, totalNetWin }) {
    $("body").append(
        `
      <div style="display:flex;justify-content:center;align-items:center; background:transparent;" data-v-1dcba851="" class="msg">
      <img class="msg" src="/images2/winnLossPopup.png" width="350px"/>
      <div style="text-transform:capitalize;position: absolute; top: 40%; left: 50%; transform: translate(-50%, -50%); color: black;text-align: center;font-size:25px">
          ${gameName == 1 ? '5d 1min' : gameName == 3 ? '5d 3min' : gameName == 5 ? '5d 5min' : gameName == 10 ? '5d 10min' : '5d '}
      </div>
      <div style="position: absolute; top: 69%; left: 50%; transform: translate(-50%, -50%); color: white;text-align: center;">
          <div style="color:black;display:flex;justify-content:space-between;align-items:center;font-size:14px">
              <div style="margin-right: 30px;">Total Bet: </div>
              <div>${totalBet} </div>
          </div>
      </div>
      <div style="position: absolute; top: 78%; left: 50%; transform: translate(-50%, -50%); color: white;text-align: center;">
      <div style="color:black;display:flex;justify-content:space-between;align-items:center;font-size:14px">
      <div style="margin-right: 30px;">
          ${totalNetWin > 0 ? 'Total Win: ' : 'Total Loss: '}
      </div>
      <div style="color: ${totalNetWin > 0 ? 'green' : 'red'}">
          ${Math.abs(totalNetWin)}
      </div>
  </div>
      </div>
  </div>
...
function callAjaxMeJoin2(data2) {
    $.ajax({
        type: "POST",
        url: "/api/webapi/5d/GetMyEmerdList",
        data: {
        ...
        success: function (response) {
            ...
            let lastPeriod = data[0].stage
            console.log(lastPeriod, data2.period)

            let isMatched = Boolean(parseInt(lastPeriod) == parseInt(data2.period));
            console.log(isMatched);

            if (isMatched) {
                // Condition is true
                const matchedOrders = data.filter(order => order.stage === lastPeriod);
                console.log(matchedOrders)

                let totalWon = 0;
                let totalBet = 0;
                let totalLost = 0;
                let gameName = data2.game;

                matchedOrders.forEach((order) => {
                    // Check if the order is a win or a loss
                    totalBet += order.money
                    if (order.status === 1) {
                        totalWon += order.get - order.money;
                    } else if (order.status === 2) {
                        totalLost += order.money;
                    }
                });
                const totalNetWin = totalWon - totalLost;

                // Prepare the message based on total net win
                let message;
                if (totalNetWin > 0) {
                    message = `Congratulations! You won ₹${totalNetWin}.`;
                } else if (totalNetWin < 0) {
                    message = `Sorry, you lost ₹${Math.abs(totalNetWin)}.`;
                } else {
                    message = "No winnings or losses.";
                }

                // Show popup with the message
                console.log("gameName", gameName)
                console.log(totalBet, "totalBet")
                WinOrLossPopup({ gameName, totalBet, totalNetWin });
            } else {
                // Condition is false
...
async function showMeJoin2(data2) {
    console.log(data2, "data2")
    await callAjaxMeJoin2(data2);
    setTimeout(() => {
        $('#showJoinMe .hb').click(function (e) {
            e.preventDefault();
            $(this).find('.details').toggleClass("display-none");
